...
def get_earliest_record(records):
    ...
    for record in records:
        if record.pubdate < earliest.pubdate:  # Date order sorting
            earliest = record
        elif record.pubdate == earliest.pubdate:
            #Journal name resolves ambiguity in sorting equivalent dates
...
def main():
    ...
    #Now we can do search, first by date, then by abbreviated journal name
